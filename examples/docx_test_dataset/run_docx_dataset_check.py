...
PREFERRED_SKILLS = ["Tableau", "NumPy", "scikit-learn"]


def create_job():
    """Return the job dictionary used by the DOCX test dataset."""
    ...
    return {
        ...
        "required_education": "Bachelor's degree in a quantitative field",
...
def analyse_resume(docx_path, job, sbert_model, name_model):
    """Run one DOCX through the complete SmartATS scoring pipeline."""

    extraction = extract_resume_text(docx_path.name, docx_path.read_bytes())
    if not extraction["success"]:
        return {"file": docx_path.name, "error": extraction["message"]}

    clean_text = clean_resume_text(extraction["text"])
    section_result = detect_sections(clean_text)
    sections = section_result["sections"]
    contact = extract_candidate_contact(extraction["text"], docx_path.name, name_model)
...
def main():
    """Analyse all DOCX resumes and print their descending score order."""
    ...
    for docx_path in sorted(DATASET_FOLDER.glob("*.docx")):
        results.append(analyse_resume(docx_path, job, sbert_model, name_model))

    results.sort(key=lambda item: item.get("overall", 0), reverse=True)
    print("\nSmartATS DOCX test dataset results")
    print("=" * 96)
